...
```{r}
# correlation
cor.test(wc_snodas_snotel_wint$snodas_snow_depth_mm, wc_snodas_snotel_wint$snotel_depth_mm)

# linear model 
summary(lm(wc_snodas_snotel_wint$snodas_snow_depth_mm ~ wc_snodas_snotel_wint$snotel_depth_mm))

# root mean square error
rmse(wc_snodas_snotel_wint$snodas_snow_depth_mm, wc_snodas_snotel_wint$snotel_depth_mm, na.rm = TRUE)
```
...
wc_prism_snotel_wint_r2 <-  wc_prism_snotel_wint %>% 
  ...
  mutate(prism_snow_snotel_swe_p = summary(lm(prism_snow_mm_accum ~
                                                 snotel_swe_mm))$coefficients[2,4]) %>% 
  slice(1) %>% 
  mutate(month_name = month(Date, label = TRUE, abbr=TRUE)) %>% 
  select(month_name, prism_snow_snotel_swe_r2, prism_snow_snotel_swe_p)
...
# rsquared for each WY
bs_prism_snotel_wint_r2_wy <-  bs_prism_snotel_wint %>% 
  ungroup() %>% 
  group_by(waterYear) %>% 
  mutate(prism_snow_snotel_swe_r2 = summary(lm(prism_snow_mm_accum ~
                                                 snotel_swe_mm))$adj.r.squared) %>% 
